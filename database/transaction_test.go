...
func TestMulti(t *testing.T) {
	...
	assert.AssertMultiBulkReply(t, result, []string{value})
...
func TestSyntaxErr(t *testing.T) {
	...
	assert.AssertErrReply(t, result, "ERR wrong number of arguments for 'set' command")
	testServer.Exec(conn, utils.ToCmdLine("get", "a"))
	result = testServer.Exec(conn, utils.ToCmdLine("exec"))
	assert.AssertErrReply(t, result, "EXECABORT Transaction discarded because of previous errors.")
