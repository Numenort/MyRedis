...
func TestZRank(t *testing.T) {
	...
	for i, member := range members {
		...
		assert.AssertIntReply(t, result, i)
		result = testDB.Exec(nil, utils.ToCmdLine("ZRevRank", key, member))
		assert.AssertIntReply(t, result, size-i-1)
	}
}
...
func TestZRange(t *testing.T) {
	...
	assert.AssertMultiBulkReply(t, result, members[0:10])
	result = testDB.Exec(nil, utils.ToCmdLine("ZRange", key, start, end, "WITHSCORES"))
	assert.AssertMultiBulkReplySize(t, result, 20)
	result = testDB.Exec(nil, utils.ToCmdLine("ZRevRange", key, start, end))
	assert.AssertMultiBulkReply(t, result, reverseMembers[0:10])

	start = "0"
	end = "200"
	result = testDB.Exec(nil, utils.ToCmdLine("ZRange", key, start, end))
	assert.AssertMultiBulkReply(t, result, members)
	result = testDB.Exec(nil, utils.ToCmdLine("ZRevRange", key, start, end))
	assert.AssertMultiBulkReply(t, result, reverseMembers)

	start = "0"
	end = "-10"
	result = testDB.Exec(nil, utils.ToCmdLine("ZRange", key, start, end))
	assert.AssertMultiBulkReply(t, result, members[0:size-10+1])
	result = testDB.Exec(nil, utils.ToCmdLine("ZRevRange", key, start, end))
	assert.AssertMultiBulkReply(t, result, reverseMembers[0:size-10+1])

	start = "0"
	end = "-200"
	result = testDB.Exec(nil, utils.ToCmdLine("ZRange", key, start, end))
	assert.AssertMultiBulkReply(t, result, members[0:0])
	result = testDB.Exec(nil, utils.ToCmdLine("ZRevRange", key, start, end))
	assert.AssertMultiBulkReply(t, result, reverseMembers[0:0])

	start = "-10"
	end = "-1"
	result = testDB.Exec(nil, utils.ToCmdLine("ZRange", key, start, end))
	assert.AssertMultiBulkReply(t, result, members[90:])
	result = testDB.Exec(nil, utils.ToCmdLine("ZRevRange", key, start, end))
	assert.AssertMultiBulkReply(t, result, reverseMembers[90:])
}
...
func TestZRangeByScore(t *testing.T) {
	...
	assert.AssertMultiBulkReply(t, result, members[20:31])
	result = testDB.Exec(nil, utils.ToCmdLine("ZRangeByScore", key, min, max, "WithScores"))
	assert.AssertMultiBulkReplySize(t, result, 22)
	result = testDB.Exec(nil, utils.ToCmdLine("ZRevRangeByScore", key, max, min))
	assert.AssertMultiBulkReply(t, result, reverse(members[20:31]))

	min = "-10"
	max = "10"
	result = testDB.Exec(nil, utils.ToCmdLine("ZRangeByScore", key, min, max))
	assert.AssertMultiBulkReply(t, result, members[0:11])
	result = testDB.Exec(nil, utils.ToCmdLine("ZRevRangeByScore", key, max, min))
	assert.AssertMultiBulkReply(t, result, reverse(members[0:11]))
	...
	assert.AssertMultiBulkReply(t, result, members[90:])
	result = testDB.Exec(nil, utils.ToCmdLine("ZRevRangeByScore", key, max, min))
	assert.AssertMultiBulkReply(t, result, reverse(members[90:]))

	min = "(20"
	max = "(30"
	result = testDB.Exec(nil, utils.ToCmdLine("ZRangeByScore", key, min, max))
	assert.AssertMultiBulkReply(t, result, members[21:30])
	result = testDB.Exec(nil, utils.ToCmdLine("ZRevRangeByScore", key, max, min))
	assert.AssertMultiBulkReply(t, result, reverse(members[21:30]))

	min = "20"
	max = "40"
	result = testDB.Exec(nil, utils.ToCmdLine("ZRangeByScore", key, min, max, "LIMIT", "5", "5"))
	assert.AssertMultiBulkReply(t, result, members[25:30])
	result = testDB.Exec(nil, utils.ToCmdLine("ZRevRangeByScore", key, max, min, "LIMIT", "5", "5"))
	assert.AssertMultiBulkReply(t, result, reverse(members[31:36]))
}
...
func TestZRem(t *testing.T) {
	...
	assert.AssertIntReply(t, result, 10)
	result = testDB.Exec(nil, utils.ToCmdLine("zcard", key))
	assert.AssertIntReply(t, result, size-10)
...
func TestZCount(t *testing.T) {
	...
	assert.AssertIntReply(t, result, 11)
	...
	max = "(30"
	result = testDB.Exec(nil, utils.ToCmdLine("zcount", key, min, max))
	assert.AssertIntReply(t, result, 9)
}
...
func TestZIncrBy(t *testing.T) {
	...
	assert.AssertBulkReply(t, result, "10")
	result = testDB.Exec(nil, utils.ToCmdLine("ZIncrBy", key, "10", "a"))
	assert.AssertBulkReply(t, result, "20")
...
func TestZPopMin(t *testing.T) {
	...
	assert.AssertMultiBulkReply(t, result, []string{"a", "1", "b", "1"})
	result = testDB.Exec(nil, utils.ToCmdLine("ZRange", key, "0", "-1"))
	assert.AssertMultiBulkReply(t, result, []string{"c"})
	...
	assert.AssertErrReply(t, result, "WRONGTYPE Operation against a key holding the wrong kind of value")
}
...
func TestZRemRangeByLex(t *testing.T) {
	...
	key := utils.RandString(10)
	// a b c d e
	assert.AssertNotError(t, testDB.Exec(nil, utils.ToCmdLine("ZAdd", key, "0", "e", "0", "d", "0", "c", "0", "b", "0", "a")))

	// case1
	assert.AssertIntReply(t,
		testDB.Exec(nil, utils.ToCmdLine("ZRemRangeByLex", key, "-", "+")),
		5)

	assert.AssertMultiBulkReplySize(t,
