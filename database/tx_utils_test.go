...
func TestRollbackToList(t *testing.T) {
	...
	assert.AssertMultiBulkReply(t, actual, []string{value, value2})
}
...
func TestRollbackToSet(t *testing.T) {
	...
	assert.AssertMultiBulkReply(t, actual, []string{value})
}
...
func TestRollbackSetMembers(t *testing.T) {
	...
	assert.AssertStatusReply(t, actual, "none")
}
...
func TestRollbackHashFields(t *testing.T) {
	...
	testDB.Remove(key)
	testDB.Exec(nil, utils.ToCmdLine("HSet", key, value, value))
	undoCmdLines := rollbackHSetFields(testDB, key, value, value2)
	...
	assert.AssertNullBulk(t, actual)

	testDB.Remove(key)
	undoCmdLines = rollbackHSetFields(testDB, key, value)
	testDB.Exec(nil, utils.ToCmdLine("HSet", key, value, value))
	for _, cmdLine := range undoCmdLines {
		testDB.Exec(nil, cmdLine)
	}
...
func TestRollbackToZSet(t *testing.T) {
	...
	assert.AssertBulkReply(t, actual, "1")
}
